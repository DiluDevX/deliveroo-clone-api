...
type UpdateRestaurantInput = z.infer<typeof updateRestaurantFullyRequestBodySchema>;
type PartialUpdateRestaurantInput = z.infer<typeof updateRestaurantPartiallyRequestBodySchema>;
...
const findOne = async (orgID: string) => {
  const input = mongoose.Types.ObjectId.isValid(orgID) ? { _id: orgID } : { name: orgID };
...
const findAndUpdatePartially = async (id: string, data: PartialUpdateRestaurantInput) => {
  return Restaurant.findByIdAndUpdate(id, data, { new: true });
};
